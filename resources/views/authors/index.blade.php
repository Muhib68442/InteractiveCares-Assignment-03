...
@section('page_subtitle', 'Manage your book authors')
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Name
                        </th>
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr class="hover:bg-gray-50 transition-colors">
                        @foreach ($authors as $data)
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600">{{ $data->id }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        ...
                                        $color = $data->status == 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $color }}">{{ $data->status }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <form action="{{ route('authors.destroy', $data->id) }}" method="post">
                                            @csrf
                                            @method('delete')
